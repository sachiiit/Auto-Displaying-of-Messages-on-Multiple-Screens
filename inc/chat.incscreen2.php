...
class SimpleChat {
    ...
    function getMessages() {
        ...
        $vRes = mysql_query("SELECT * FROM `s_chat_messages` where screen='screen2' ORDER BY `when` DESC LIMIT 8");
		...
		$sMessages2 = 'Topics:    ';
		$ii=0;
		$vRes2 = mysql_query("SELECT DISTINCT `topictitle` FROM `s_chat_messages` where screen='screen2' ORDER BY ID DESC LIMIT 8");
		 if ($vRes2) 
		 {
		...
		{
			//echo $x;
			$query1="SELECT COUNT(*) FROM `s_chat_messages` where screen='screen2' and `topictitle`='".$x."'";
			//echo $query1;
